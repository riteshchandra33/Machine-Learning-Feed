...
for n_clusters in range(2, 7):
    # Create the KMeans object and specify the number of clusters
    ...
    # Plot predicted clusters for the current number of clusters
    plt.figure(figsize=(12, 5))  # Create a new figure for each n_clusters
    plt.subplot(1, 2, 2)
    plt.scatter(feature_1, feature_2, c=labels_predicted, s=10, cmap='viridis')
    plt.title(f'Predicted Clusters with n_clusters={n_clusters}')
    plt.xlim(-15, 15)
    plt.ylim(-15, 15)
    plt.show()